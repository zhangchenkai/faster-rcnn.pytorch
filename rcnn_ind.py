...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset',
                        ...
                        default='zju_industry_binary', type=str)
    parser.add_argument('--net', dest='net',
                        help='vgg16, res101',
                        default='vgg16', type=str)
    parser.add_argument('--start_epoch', help='starting epoch',
                        default=1, type=int)
...
if __name__ == '__main__':
    ...
    assert args.dataset == 'zju_industry_binary'

    pattern_names = ['sl1604', 'sxl1627', ]
    for p_name in pattern_names:
        print('{0:#^64}'.format(p_name))
        ...
        args.imdb_name = "industry_binary_%s_train_supervised" % p_name
        ...
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=False
        )
        ...
        if args.resume:
            load_name = os.path.join(output_dir, 'pattern{}_s{}.pth'.format(p_name, args.session))
        ...
        if args.epochs != 0:
            save_name = os.path.join(output_dir, 'pattern{}_s{}.pth'.format(p_name, args.session))
            save_checkpoint({
        ...
        args.imdbval_name = "industry_binary_%s_test" % p_name
        ...
        ap_thresh = OrderedDict(pattern=p_name)
        ...
        os.makedirs('~/Desktop/IndustryDataset', exist_ok=True)
        df.to_csv('~/Desktop/IndustryDataset/object_rcnn_ind.csv')
