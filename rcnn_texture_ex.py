...
def parse_args():
    ...
    parser.add_argument('--start', required=True, help='start pattern id', type=int)
    parser.add_argument('--end', required=True, help='end pattern id', type=int)
...
if __name__ == '__main__':
    ...
    DATASET = 'fabric_exclusive_binary'
    P_TYPE = 'P-%d'
    P_NUM = 15
    assert 1 <= args.start <= args.end <= P_NUM
    ...
    pid_list = list(range(args.start, args.end + 1))
    print("patterns need to train: ", pid_list)
    for ex_pid in pid_list:
        # for ex_pid in [5,]:
        p_str = P_TYPE % ex_pid
        ...
        # train set
        imdb_name = DATASET + "_p%d_train_supervised" % ex_pid
        imdb, roidb, ratio_list, ratio_index = combined_roidb(imdb_name)
        train_size = len(roidb)
        print('{:d} roidb entries'.format(train_size))

        output_dir = os.path.join(args.save_dir, args.net, DATASET, 'exclusive', 'pattern-%d' % ex_pid)
        ...
        if args.resume:
            load_name = os.path.join(output_dir, 'pattern{}_s{}.pth'.format(ex_pid, args.session))
        ...
        if args.epochs != 0:
            save_name = os.path.join(output_dir, 'pattern{}_s{}.pth'.format(ex_pid, args.session))
            save_checkpoint({
        ...
        imdb_name = DATASET + "_p%d_test" % ex_pid
        ...
        ap_thresh = OrderedDict({'pattern': ex_pid})
        ...
        df.to_csv('~/Desktop/FabricDataset/rcnn_texture_ex_%d-%d.csv' % (args.start, args.end), index=False)
